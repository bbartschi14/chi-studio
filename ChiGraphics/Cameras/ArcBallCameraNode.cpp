...
void ArcBallCameraNode::Update(double delta_time) {
 ...
 /* else if (input_manager.IsRightMousePressed()) 
  ...
  auto V = make_unique<glm::mat4>(glm::lookAt(glm::vec3(0, 0, distance_), glm::vec3(0), glm::vec3(0, 1.f, 0)));

  *V *= glm::toMat4(GetTransform().GetRotation()) * glm::translate(glm::mat4(1.f), GetTransform().GetPosition());
